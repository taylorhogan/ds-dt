...
def create_graphs ():
    ...
    if full:
        ...
        for f in files:
            ...
            x = np.append(x, cur_exposures * exposure_time_min)
            y = np.append(y, mse)
            cur_exposures = cur_exposures * 2
        x = np.append(x, 640)
        y = np.append(y, 0.0)
        ...
        plt.savefig("ds-dt.png")
        plt.show()
        plt.clf()


        diff, mse, max_diff = compare_fits_files(files[0],files[1])
        stretched_data_of_diff = linear_stretch(diff)
        plot_fits_image(diff,stretched_data_of_diff, "final_stretched_difference.png")

        fit_to_3d_plot_data(diff, "final_stretched_difference_3d.png", "Difference Between 0 and 1")
        fit_to_3d_plot_data(stretched_data_of_diff, "final_stretched_difference_3d_0_1.png", "Difference Between 0 and 1")

        diff, mse, max_diff = compare_fits_files(files[0],files[4])
        stretched_data_of_diff = linear_stretch(diff)
        plot_fits_image(diff,stretched_data_of_diff, "final_stretched_difference.png")
